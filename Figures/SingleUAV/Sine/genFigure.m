...
Sol_soft=load('SingleUAV_SoftConstraint_Sine.mat');
...
plot(Sol_soft.tt, Sol_soft.x1,'--','LineWidth',2,'Color',[0.8500 0.3250 0.0980])
% ylim([problem.states.x0(2) problem.states.x0(1)])
ylim([-5 17])
hold on
% plot(tt, x2, LineWidth=2)
plot(tt, problem.data.XT(tt), 'k--')
...
patch([tt' fliplr(tt')], [y1' min(ylim).*ones(size(y1'))], [.6 .6 .6], 'FaceAlpha', .3 )        % Below Lower Curve
patch([tt' fliplr(tt')], [y2' max(ylim).*ones(size(y2'))], [.6 .6 .6], 'FaceAlpha', .3)        % Above Upper Curve
hold off 
xlim([0 tt(end)])
legend(["Set-point Tracking", "In-range Tracking", "with Soft Constraints", "Reference", "Out-of-range Region"],'Location','northoutside','NumColumns',2,'Interpreter','latex','FontSize',11)
