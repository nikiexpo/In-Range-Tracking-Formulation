...
problem.time.tf_min=1600;     
problem.time.tf_max=1600; 
guess.tf=1600;
...
problem.states.x0=[25 -20 0 0 80 80];
...
k_R=100+(1.6e06-100)*(k_L-data.penalty.values(1))./(data.penalty.values(end)-data.penalty.values(1));
% k_R=1.6e06*k_L;
% k_R=1.6e09;
...
% JR1=1/k_R.*(x1-x_t).^2;
% JR2=1/k_R.*(x2-x_t).^2;
JR1=1/k_R.*soft_max(((x1-x_t).^2-data.delta.^2)./data.delta.^2,0,3);
JR2=1/k_R.*soft_max(((x2-x_t).^2-data.delta.^2)./data.delta.^2,0,3);
J1=JL1+JR1;
J2=JL2+JR2;
% variable_k=1./(J1-J2+0.1);
% stageCost = -soft_max( -J1, -J2, variable_k);
% stageCost = min( (J1), (J2));
stageCost = soft_max(-soft_max( -J1, -J2,3),-1.5,3);
