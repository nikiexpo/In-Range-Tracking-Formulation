...
lx = linspace(solution.X(1,1), solution.X(end,1), length(xpos))';
ly = linspace(solution.X(1,2), solution.X(end,2), length(ypos))';
% X = [xpos , fliplr(xt)];
% Y = [ypos , fliplr(yt-(40/130).*zpos)];
% Z = [zpos, fliplr(0.*zpos)];
X = [xpos , fliplr(xpos)];
Y = [ypos+(40/130).*zpos , fliplr(ypos)];
Z = [zt, fliplr(zpos)];
% patch(X,Y,"red", FaceAlpha=0.3)
% X = [xpos , fliplr(xpos)];
% Y = [ypos+(40/130).*zpos , fliplr(ypos)];
% Z = [zt, fliplr(zpos)];
% fill3(X,Y,Z,"red", FaceAlpha=0.3)
plot3(xt,yt,zt, Color=[0 0 0], LineWidth=3)
