...
%known
problem.data.phi = 3.11*10^-5; % kgm2
problem.data.KT = 7.62*10^-3; % m sqrt(J kg-1 K-1)
problem.data.VCC = 4.2*10-4; % m3
problem.data.betag = 0.0396; % sqrt(kg K J-1)
problem.data.betaa = 0.0404; % sqrt(kg K J-1)
problem.data.A1 = 1.116*10^-4; % m2
problem.data.A2 = 3.879*10^-4; % m2
problem.data.kappag = 1.333;
problem.data.kappaa = 1.4;
problem.data.etam = 0.99;
problem.data.Ha = 42000; % kJ kg-1
problem.data.Rg = 288;  % J kg-1 K-1
problem.data.cpg = 1152.9; % J kg-1 K-1
problem.data.cpa = 1005.9; % J kg-1 K-1
problem.data.cv = 864.9; % J kg-1 K-1
problem.data.etaT = 0.86;
problem.data.etaCC = 0.95;
problem.data.delta = 0.01;
problem.data.sigmaCC = 0.95;
problem.data.sigmaD = 0.99;

%measured
problem.data.n=113900; %rpm
problem.data.qlambda1 = 0.289;
problem.data.qlabmda3 = 0.904;
problem.data.PC = 6441.5; % W
problem.data.PT = 6560.8; % W
problem.data.T1 = 283; %  K
problem.data.T2 = 365.9; % K
problem.data.T3 = 995.6; % K
problem.data.T4 = 923.3; % K
problem.data.etaC = 0.72; 
problem.data.p1 = 101.85; %kpa
problem.data.p3 = 194.12; %kpa
problem.data.p5 = 102.88; %kpa
problem.data.piC = 1.954;
problem.data.piT = 1.906;
problem.data.massflow2 = 0.07726; %kg/s
problem.data.bdash = 0.07869; %kg/s
problem.data.massflowfuel = 0.001431; %kg/s
problem.data.mcc = 2.84*10^-4; %kg
